...
__device__ void get_WD(FileSystem * fs, int *WD) {
  for (int i =0; i < 1024; i++) {
    if (VALID(fs->FCBS[i].address)) {
      if (WD(fs->FCBS[i].address)) {
        WD[0] = i;
        if (!ROOT(fs->FCBS[i].address)) {
          WD[1] = PARENT(fs->FCBS[i].address);
        }
        else {
          WD[1] = -1;
        }
...
__device__ void rm_DIR(FileSystem *fs, int block) {
  int queue[1024];
  if (!ROOT(fs->FCBS[block].address)) 
  fs->FCBS[PARENT(fs->FCBS[block].address)].size -= get_len(fs->FCBS[block].name);
  RESET_VALID(fs->FCBS[block].address);
  ...
  while (queue[cur_ptr] != -1) {
    ...
    for (int i = 0; i < 1024; i++) {
      if (VALID(fs->FCBS[i].address)) {
        if (!ROOT(fs->FCBS[i].address)) {
          if (PARENT(fs->FCBS[i].address) == cur_idx) {
            if (DIR(fs->FCBS[i].address)) {
              queue[ptr++] = i;
            }
            else {
              flush_blocks(fs, get_address(fs->FCBS[i]), ceil(fs->FCBS[i].size, 32));
              RESET(fs->FCBS[i].address);
            }
          }
        }
      }
    }
    flush_blocks(fs, get_address(fs->FCBS[cur_idx]), ceil(fs->FCBS[cur_idx].size, 32));
    RESET(fs->FCBS[cur_idx].address);
  }
}
